(() => {
  ...
  /** Challenge id → display name (Grounded mutations). */
  const DISPLAY_NAMES = {
  ...
  function parsePerkComponent(rawPlayer) {
    ...
    let off = nameAt + PERK_PATH.length + 1;
    ...
    off += 4;
    if (count < 1 || count > 80) return { ok: false, entries: [] };
    const entries = [];
    for (let i = 0; i < count; i++) {
      const fs = readFString(buf, off);
      if (!fs || fs.next + 12 > buf.length) {
  ...
  function unlockAllMutations(rawPlayer, phase) {
    const target = phase == null ? MAX_PHASE : Math.max(0, Math.min(MAX_PHASE, Math.floor(Number(phase))));
