(() => {
  ...
  function skipShortName(buf, pathAt, pathStr) {
    ...
    if (len > 1 && len < 64 && o + 4 + len <= buf.length) {
      return o + 4 + len;
    }
    return o;
  }
  ...
  function invBounds(buf) {
    ...
    // G2: /Script/Maine.InventoryComponent + FString "InventoryComponent" then payload
    const afterName = skipShortName(buf, invAt, INV_PATH);
    if (afterName + 5 > buf.length) return null;
    // Observed: optional pad 0x00, then u32 count
    let countOff = afterName;
    let count = C.readU32(buf, countOff);
    if ((count > 500 || count === 0) && buf[afterName] === 0) {
      countOff = afterName + 1;
      count = C.readU32(buf, countOff);
    }
    if (count < 1 || count > 500) return null;
    const end = eqAt > invAt ? eqAt : buf.length;
    return { invAt, dataAt: afterName, countOff, count, end };
  }
